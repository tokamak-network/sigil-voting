...
export const en: Translations = {
  ...
  landing: {
    ...
    testnetNote: 'SDK on NPM · 493 tests',
    ...
    demo: {
      title: 'Demo Video',
      subtitle: 'A short walkthrough of the full private voting flow.',
    ...
    transparency: {
      ...
      visibleItems: [
        { title: 'Encrypted Messages', desc: 'Ciphertext posted on-chain (meaningless number sequences).' },
        { title: 'Aggregate Results', desc: 'Total For / Against published after tallying.' },
        { title: 'ZK Proofs', desc: 'Cryptographic evidence that the tally is correct.' },
      ],
      hiddenItems: [
        { title: 'Individual Vote Choices', desc: 'No on-chain record of who voted what.' },
        { title: 'Who Voted Which Way', desc: 'Identity and choice are never linked publicly.' },
        { title: 'Who Changed Their Key and Re‑Voted', desc: 'Key-change history is not publicly distinguishable.' },
      ],
  ...
  tallying: {
    ...
    choice: 'Choice',
    spent: 'Spent',
    participation: 'Vote Messages',
    totalUsers: 'Registered Voters',
  ...
  proposals: {
    ...
    statusEnded: 'Ended',
    searchPlaceholder: 'Search by proposal title or description...',
  },
  voteHistory: {
    ...
    pageSubtitle: 'Proposals you participated in',
    emptyTitle: 'No votes yet',
    emptyDesc: 'Go to proposals and cast your first vote.',
    goToProposals: 'Go to Proposals',
    votedOn: 'Voted on',
    status: 'Status',
    myVote: 'My Vote',
    result: 'Result',
    connectWallet: 'Connect your wallet to view your vote history.',
    nav: 'Vote History',
  },
  myVote: {
    title: 'My Vote',
  ...
  voteErrors: {
    ...
    alreadyVoted: 'Already voted. You can change your key to re-vote',
    genericError: 'Vote submission failed. Please try again',
    timeout: 'Transaction timed out',
    encryption: 'Encryption error occurred',
  },
  technology: {
    nav: 'Technology',
    ...
    zkVoting: {
      ...
      desc: 'Votes are encrypted client-side and stored on-chain as ciphertext. The message queue is hashed with Poseidon, and the tally is verified by Groth16 proofs — without any reveal phase. Individual votes remain private.',
      howTitle: 'How It Works',
      howDesc: 'When you vote, the command (choice, weight, pollId, nonce) is packed and encrypted in the browser using ECDH + Poseidon-based DuplexSponge. Encrypted messages are stored in the on-chain AccQueue. Eligibility and voting power are enforced via on-chain registration and the voice credit proxy, and the tally is verified with Groth16 proofs — without exposing your vote.',
      point1: 'Only ciphertext is stored on-chain (and hashed into AccQueue leaves)',
      ...
      commitLabel: 'AccQueue Leaf (On-Chain)',
      commitFormula: 'Poseidon(encMessage[0..9], encPubKeyX, encPubKeyY)',
    ...
    infrastructure: {
      ...
      delegation: { title: 'Delegation Registry', desc: 'Delegation is supported. Full voting-power delegation requires DelegatingVoiceCreditProxy and depends on deployment config.' },
      execution: { title: 'Timelocked Execution', desc: 'Optional on-chain execution when the executor is configured and execution is registered.' },
    },
    cta: {
  ...
  governance: {
    delegation: {
      ...
      lockedDesc: 'You cannot vote directly while delegating.',
      lockedCta: 'Remove delegation to vote',
      effectNote: 'Delegation is applied when voice credits are calculated at signUp. Polls that already started or have closed registration may not update immediately. Delegation changes apply starting with the next proposal.',
      effectNoteLimited: 'In the current deployment, delegation does not affect voting credits. It only applies to proposal creation thresholds. Delegation changes apply starting with the next proposal.',
      manage: 'Manage',
